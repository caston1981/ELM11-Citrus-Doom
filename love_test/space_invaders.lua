...
UPDATE_DELAY = 1/30  -- Update 30 times per second
...
update_timer = 0  -- Timer for controlling update rate
...
function init_game(new_level)
    ...
    local alien_speed = ALIEN_SPEED + (level - 1) * 0.5  -- Aliens move faster each level
    local alien_shoot_chance = ALIEN_BULLET_CHANCE + (level - 1) * 0.002  -- Aliens shoot more frequently
    local move_delay_level = math.max(15, move_delay - (level - 1) * 2)  -- Aliens move more frequently
...
function update_game()
    ...
    if game_over and input.shoot then
        init_game()
        return
    end
    ...
    if move_timer >= move_delay then
        ...
        if should_change then
            ...
            for _, alien in ipairs(aliens) do
                if alien.alive then
                    alien.x = alien.x + direction * alien.speed
                end
            end
        end
    end
    ...
    -- Alien shooting
    for _, alien in ipairs(aliens) do
        if alien.alive and math.random() < alien.shoot_chance then
    ...
    if aliens_alive == 0 then
        -- All aliens defeated - advance to next level
        init_game(level + 1)
    end
end
...
function draw_game()
    ...
    if game_over then
        ...
        screen.drawText(SCREEN_WIDTH/2 - 40, SCREEN_HEIGHT/2 + 30, "Level: " .. level)
        screen.drawText(SCREEN_WIDTH/2 - 50, SCREEN_HEIGHT/2 + 50, "Press SHOOT to restart")
        return
    end
    
    -- Draw HUD (position above aliens)
    screen.setColor(255, 255, 255)
    screen.drawText(5, 5, "Score: " .. score)
    screen.drawText(150, 5, "Level: " .. level)
...
function onTick(dt)
    ...
    if update_timer >= UPDATE_DELAY then
        update_timer = update_timer - UPDATE_DELAY
        update_game()
    end
end
